...
def aggregate_data(feature,bin_width):
    """
    Aggregate all the strains and mouses with any feature together in one dataframe. It will return a dataframe
    ...
    for i in range(3):
        for j in range(4):
            tmp = aggegate_interval(strain = i,mouse = j,feature = feature,bin_width = bin_width)
            tmp["strain"] = i
            tmp["mouse"] = j
...
def mix_strain(data,feature):
    ...
    Returns:
    ----------
    Two mixed model regression results which includes all the coefficients, t statistics and p values for corresponding coefficients; The first model includes interaction terms while the second model does not include the interaction terms
    
    Likelihood ratio test p values, if it is below our significance level, we can conclude that the different strains have significantly different time patterns
    ...
    data.columns = names
    print(data)
    md1 = smf.mixedlm("feature ~ hour + strain0 +strain1 + strain0*hour+ strain1*hour", data, groups=data["mouse"])
    mdf1 = md1.fit()
    like1 = mdf1.llf
    print(mdf1.summary())
    md2 = smf.mixedlm("feature ~ hour + strain0 +strain1", data, groups=data["mouse"])
    mdf2 = md2.fit()
    like2 = mdf2.llf
    print(mdf1.summary())
    fstat = like1-like2
